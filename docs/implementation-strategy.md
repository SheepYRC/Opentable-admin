...
通过分析设计文档中的愿景、架构与路线图，结合目前已经稳固的“管理后台地基”，本项目正处于从 **“通用脚手架”** 转向 **“专业数据 IDE”** 的关键拐点。
...
1.  **初始化 `src/core` 目录**：作为整个项目的计算核心，不要将数据处理逻辑混在 Vue 组件中。
    ...
    >- 实现 `core/importer.ts`：封装文件导入逻辑，支持 CSV/Parquet。
2.  **实现“拖拽即看” MVP**：
    -   在侧边栏新加一个菜单，点击打开“数据视图”页面 `views/viewer/index.vue` ，用于支持导入和显示数据。
    >- 已添加
